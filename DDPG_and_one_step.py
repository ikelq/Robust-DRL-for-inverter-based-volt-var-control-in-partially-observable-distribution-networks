...
class DPGAgent:
    ...
    def update_model_DDPG(self) -> Tuple[torch.Tensor, ...]:
        ...
        q_pl_next_pred = self.qf_pl_target1(next_state, self.actor_target(next_state))

        q_pl_target = reward[:, 0].reshape(-1, 1) + 50*reward[:, 1].reshape(-1, 1) + self.gamma * mask * q_pl_next_pred
        ...
        actor_loss = -self.qf_pl_1(state, self.actor(state)).mean()
        self.actor_optimizer.zero_grad()
        actor_loss.backward()
        self.actor_optimizer.step()
        self._target_soft_update_DDPG()

        return actor_loss.detach().cpu().numpy(), 0, qf_pl_1_loss.detach().cpu().numpy()
    ...
    def train(self, num_frames: int, plotting_interval: int = 400, safepolicy=True, virtualinteraction=True):
        ...
        data_train.to_csv('train' + str(self.env.name) + str(self.seed) + str(self.gamma) + '.csv')
        ...
        data_train_test.to_csv('traintest' + str(self.env.name) + str(self.seed)  + str(self.gamma) + '.csv')
        ...
        data_train_loss.to_csv(
            'trainloss' + str(self.env.name) + str(self.seed) + str(self.gamma)+ '.csv')
    ...
    def test(self, test_frams):
        ...
        data_test.to_csv('test'+str(self.env.name)+str(self.seed) +str(self.gamma)+'.csv')
        ...
        data_test_step.to_csv('test_step' +str(self.env.name) + str(self.seed)+ str(self.gamma) + '.csv')
