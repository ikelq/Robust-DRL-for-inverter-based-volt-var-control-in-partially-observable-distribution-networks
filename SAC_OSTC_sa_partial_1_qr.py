...
import pandas as pd
import Env_partical_state_15 as Env
...
class ReplayBuffer:
    ...
    def store_1(self,
                rew: float):
        self.rews_buf[self.ptr-decay:self.ptr-1, :] = rew
...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        q2_target = reward[:, 1].reshape(-1, 1)
        ...
        if self.total_step % self.policy_update_freq == 0:
            # actor loss
            advantage = self.qf_1(state, self.actor(state)[0]) + 50*self.qf_2(state, self.actor(state)[0])
    ...
    def train(self, num_frames: int, plotting_interval: int = 400):
        ...
        for self.total_step in range(1, num_frames + 1):
            ...
            reward_5 += reward
            if self.env.step_n % decay == 0:
                self.memory.store_1(reward_5/decay)
                reward_5 = 0
        ...
        data_train.to_csv('trainsac' + str(self.env.name) + str(self.seed) + str(self.partical)+'tootsa'+str(decay)+'sqr.csv')
        ...
        data_train_test.to_csv('traintestsac' + str(self.env.name) + str(self.seed) + str(self.partical)+'tootsa'+str(decay)+'sqr.csv')
        ...
        data_train_loss.to_csv('trainlosssac' + str(self.env.name) + str(self.seed) + str(self.partical)+'tootsa'+str(decay)+'sqr.csv')
        #
