...
#         """Initializate."""
#         self.obs_buf = np.zeros([size, decay_average,  obs_dim], dtype=np.float32)
#         self.next_obs_buf = np.zeros([size, decay_average,  obs_dim], dtype=np.float32)
#         self.acts_buf = np.zeros([size, decay_average,  action_dim], dtype=np.float32)
#         self.rews_buf = np.zeros([size, decay_average, 2], dtype=np.float32)
#         self.done_buf = np.zeros([size, decay_average, 1], dtype=np.float32)
...
#
#     def store_1(self, rew: float):
#         self.rews_buf[self.ptr-decay_average:self.ptr-1, :] = rew
#
...
class ReplayBuffer:
    ...
    def store_1(self,
                rew: float):
        self.rews_buf[self.ptr-decay_average:self.ptr, :] = rew

    def sample_batch(self) -> Dict[str, np.ndarray]:
        """Randomly sample a batch of experiences from memory."""
        idxs = np.random.choice(self.size-decay_average, size=self.batch_size, replace=False)
        return dict(obs=self.obs_buf[idxs],
                    next_obs=self.next_obs_buf[idxs],
                    acts=self.acts_buf[idxs],
                    rews=self.rews_buf[idxs],
...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        q1_target = reward[:, 0]
        q2_target = 50*reward[:, 1]
        # qf_1_loss = 0.5*F.mse_loss(q_1_pred.mean(1), q1_target.mean(1).reshape(-1, 1).detach()) + 0.5*F.mse_loss(q_1_pred, q1_target.unsqueeze(2).detach())
        qf_1_loss = F.mse_loss(q_1_pred, q1_target.unsqueeze(1).detach())
        if qr:
            errors = q2_target.mean(1).reshape(-1, 1).detach() - q_2_pred
            qf_2_loss = torch.max((0.05 - 1) * errors, 0.05 * errors).mean()
        else:
            # qf_2_loss = 0.5*F.mse_loss(q_2_pred.mean(1), q2_target.mean(1).reshape(-1, 1).detach()) + 0.5*F.mse_loss(q_2_pred, q2_target.unsqueeze(2).detach())
            qf_2_loss = F.mse_loss(q_2_pred, q2_target.unsqueeze(1).detach())
    ...
    def train(self, num_frames: int, plotting_interval: int = 400):
        ...
        for self.total_step in range(1, num_frames + 1):
            ...
            # if (self.env.step_n-1) % decay_average == 0:
            #     if non_first_interval:
            #         # self.transition = [stateB, actionB, rewardB, next_stateB, doneB]
            #         self.transition = [stateB, actionB, np.resize(rewardB.mean(0), rewardB.shape), next_stateB, doneB]
            #         # np.resize(rewardB.mean(0), rewardB.shape)
            #         self.memory.store(*self.transition)
            #     stateB, actionB, rewardB, next_stateB, doneB = self.state, action, reward, next_state, done
            #     non_first_interval = 1
            # else:
            #     if non_first_interval:
            #         stateB, actionB, rewardB, next_stateB, doneB = \
            #             np.vstack((stateB, self.state)), np.vstack((actionB, action)), np.vstack(
            #                 (rewardB, reward)), np.vstack((next_stateB, next_state)), np.vstack((doneB, done))
            ...
            if self.env.step_n > 96*100*15:
                self.env.step_n = 0
            # state = next_state
            self.state = new_state
            score += reward[0]+50*reward[1]
        ...
        data_train.to_csv('trainsac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+'me1.csv')
        ...
        data_train_test.to_csv('traintestsac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+'me1.csv')
        ...
        data_train_loss.to_csv('trainlosssac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+'me1.csv')
        #
...
if __name__ == "__main__":
    ...
    decay = 15    # decay time for sampling data form data buffer
    decay_average = 15  # eahc dacay_average_reward data have the same reward
    # parameters
    num_frames = 96*100*15
    # test_frames = 96*360
    memory_size = 96*100*15
    batch_size = 128
    initial_random_steps = 96*15*2


    # partical = 1 : partical power loss; partical = 2 : partical power loss and voltage violation;
    # partical = 3 : partical state power loss voltage violation   # partical = 4 : partical state #partical = 5 partical state, reduce loss
    partical = 4

    if partical == 0:
        import Env as Env
    if partical == 2:
        import Env_partical_reward_voltage as Env
    if partical == 4:
        import Env_partical_state as Env
    if partical == 5:
        import Env_partical_state_reduce_loss as Env
