...
class SACAgent:
    ...
    def select_action(self, state: np.ndarray) -> np.ndarray:
        ...
        if self.total_step < self.initial_random_steps and not self.is_test:
            ...
            selected_action = 0.1*np.random.randn(self.action_dim)
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        qf_1_loss = two_loss_factor*F.l1_loss(q_1_pred.mean(1), q1_target.mean(1).reshape(-1, 1).detach()) \
                    + (1-two_loss_factor)*F.l1_loss(q_1_pred, q1_target.unsqueeze(2).detach())
        # qf_1_loss = F.mse_loss(q_1_pred, q1_target.unsqueeze(2).detach())
        if qr:
            errors = q2_target.detach() - q_2_pred
            qf_2_loss = torch.max((0.05 - 1) * errors, 0.05 * errors).mean()
        else:
            qf_2_loss = two_loss_factor*F.l1_loss(q_2_pred.mean(1), q2_target.mean(1).reshape(-1, 1).detach()) \
                         + (1-two_loss_factor)*F.l1_loss(q_2_pred, q2_target.unsqueeze(2).detach())
            # qf_2_loss = F.mse_loss(q_2_pred, q2_target.unsqueeze(2).detach())
    ...
    def train(self, num_frames: int, plotting_interval: int = 400):
        ...
        data_train.to_csv('trainsac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+str(two_loss_factor)+'.csv')
        ...
        data_train_test.to_csv('traintestsac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+str(two_loss_factor)+'me.csv')
        ...
        data_train_loss.to_csv('trainlosssac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+str(two_loss_factor)+'me.csv')
        #
