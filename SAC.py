...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        q_target = reward[:, 0].reshape(-1, 1) + 50*reward[:, 1].reshape(-1, 1)+ self.gamma * mask * (q_pred - alpha * log_prob)
    ...
    def train(self, num_frames: int, plotting_interval: int = 400):
        ...
        data_train.to_csv('trainsac' + str(self.env.name) + str(self.seed) + str(self.gamma) + '.csv')
        ...
        data_train_test.to_csv('traintestsac' + str(self.env.name) + str(self.seed) + str(self.gamma) + '.csv')
        ...
        data_train_loss.to_csv('trainlosssac' + str(self.env.name) + str(self.seed) + str(self.gamma) + '.csv')
    ...
    def test(self, test_frams):
        ...
        data_test.to_csv('testsac'+str(self.env.name)+str(self.seed) + str(self.gamma) + '.csv')
        ...
        data_test_step.to_csv('testsac_step' +str(self.env.name) + str(self.seed) + str(self.gamma) + '.csv')
        data_test_step_action = DataFrame(actions)
        data_test_step_action.to_csv(
            'testsac_step_action' + str(self.env.name) + str(self.seed) + str(self.gamma) + '.csv')
