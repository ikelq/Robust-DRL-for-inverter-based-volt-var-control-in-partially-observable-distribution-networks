...
class DPGAgent:
    ...
    def __init__(
        ...
        # q function
        self.qv = Critic(obs_dim + self.action_dim).to(self.device)
        self.qv_target = Critic(obs_dim + self.action_dim).to(self.device)
        self.qv_target.load_state_dict(self.qv.state_dict())

        # q powerloss function
        self.qp = Critic(obs_dim + self.action_dim).to(self.device)
        self.qp_target = Critic(obs_dim + self.action_dim).to(self.device)
        self.qp_target.load_state_dict(self.qp.state_dict())

        # optimizers
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=1e-4)
        self.qv_optimizer = optim.Adam(self.qv.parameters(), lr=3e-4)
        self.qp_optimizer = optim.Adam(self.qp.parameters(), lr=3e-4)
    ...
    def update_model_DDPG(self) -> Tuple[torch.Tensor, ...]:
        ...
        # q function loss
        mask = 1 - done
        qp_pred = self.qp(state, action)
        qv_pred = self.qv(state, action)
        # q_1_pred = self.qf_1(state, action)
        qp_next_pred = self.qp_target(next_state, self.actor_target(next_state))
        qv_next_pred = self.qv_target(next_state, self.actor_target(next_state))

        qp_target = reward[:, 0].reshape(-1, 1) + self.gamma * mask * qp_next_pred
        qv_target = reward[:, 1].reshape(-1, 1) + self.gamma * mask * qv_next_pred

        qp_loss = F.mse_loss(qp_pred, qp_target.detach())
        qv_loss = F.mse_loss(qv_pred, qv_target.detach())

        # train Q functions
        self.qp_optimizer.zero_grad()
        qp_loss.backward()
        self.qp_optimizer.step()
        self.qv_optimizer.zero_grad()
        qv_loss.backward()
        self.qv_optimizer.step()

        actor_loss = -self.qp(state, self.actor(state)).mean() - 50*self.qv(state, self.actor(state)).mean()
        self.actor_optimizer.zero_grad()
        actor_loss.backward()
        self.actor_optimizer.step()
        self._target_soft_update_DDPG()

        return actor_loss.detach().cpu().numpy(), qv_loss.detach().cpu().numpy(), qp_loss.detach().cpu().numpy()

    def _target_soft_update_DDPG(self):
        ...
        # for pl
        for t_param, l_param in zip(
                self.qp_target.parameters(), self.qp.parameters()
        ):
            t_param.data.copy_(tau * l_param.data + (1.0 - tau) * t_param.data)
        for t_param, l_param in zip(
                self.qv_target.parameters(), self.qv.parameters()
        ):
            t_param.data.copy_(tau * l_param.data + (1.0 - tau) * t_param.data)
        # for t_param, l_param in zip(
    ...
    def train(self, num_frames: int, plotting_interval: int = 400, safepolicy=True, virtualinteraction=True):
        ...
        data_train.to_csv('train' + str(self.env.name) + str(self.seed) + str(self.gamma) + 'tc.csv')
        ...
        data_train_test.to_csv('traintest' + str(self.env.name) + str(self.seed)  + str(self.gamma) + 'tc.csv')
        ...
        data_train_loss.to_csv(
            'trainloss' + str(self.env.name) + str(self.seed) + str(self.gamma)+ 'tc.csv')

        # if self.offline:
        #     torch.save(agent.log_alpha,'log_alpha_params'+str(self.env.name)+str(self.seed) + 'offline' + str(self.offline) + 'id_policy' + str(self.id_policy) + '.pth')
        #     torch.save(agent.actor.state_dict(), 'actor_p_params'+str(self.env.name)+str(self.seed)+'offline'+str(self.offline)+'id_policy'+str(self.id_policy) +'.pth')
        #     torch.save(agent.qf_1.state_dict(), 'critic1_p_params'+str(self.env.name)+str(self.seed)+'offline'+str(self.offline)+'id_policy'+str(self.id_policy)+'.pth')
        #     torch.save(agent.qf_2.state_dict(), 'critic2_p_params'+str(self.env.name)+str(self.seed)+'offline'+str(self.offline)+'id_policy'+str(self.id_policy)+'.pth')
        #     torch.save(agent.qf_target1.state_dict(), 'critic_target1_p_params'+str(self.env.name)+str(self.seed)+'offline'+str(self.offline)+'id_policy'+str(self.id_policy)+'.pth')
        #     torch.save(agent.qf_target2.state_dict(), 'critic_target2_p_params'+str(self.env.name)+str(self.seed)+'offline'+str(self.offline)+'id_policy'+str(self.id_policy)+'.pth')
    ...
    def test(self, test_frams):
        ...
        next_state, reward, done, violation, violation_M, violation_N, grid_loss, new_state = self.step_model(action)
        state = new_state
        device = self.device  # for shortening the following lines
        ...
        data_test.to_csv('test'+str(self.env.name)+str(self.seed) +str(self.gamma)+'tc.csv')
        ...
        data_test_step.to_csv('test_step' +str(self.env.name) + str(self.seed)+ str(self.gamma) + 'tc.csv')
    ...
    def _plot(
        ...
        subplot_params = [
            ...
            (247, "DPGviolation_sum_N_s", violation_sum_N_s),
