...
class ReplayBuffer:
    ...
    def store_1(self,
                rew: float):
        self.rews_buf[self.ptr-4:self.ptr-1, :] = rew

    def sample_batch(self) -> Dict[str, np.ndarray]:
        """Randomly sample a batch of experiences from memory."""
        idxs = np.random.choice(self.size-4, size=self.batch_size, replace=False)
...
class DPGAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        q_target = reward[:, 1].reshape(-1, 1)  # + self.gamma * mask * (q_pred ) # - alpha * log_prob

        qf_pl_1_loss = F.mse_loss(q_pl_1_pred, q_pl_target.detach())
        qf_1_loss = F.mse_loss(q_1_pred, q_target.detach())

        qf_loss = F.mse_loss(q_pl_1_pred + 50 * q_1_pred , q_pl_target.detach() + 50 * q_target.detach())
        ...
        return actor_loss.detach().cpu().numpy(), qf_1_loss.detach().cpu().numpy(), qf_pl_1_loss.detach().cpu().numpy(), qf_loss.detach().cpu().numpy()
    ...
    def train(self, num_frames: int, plotting_interval: int = 400, safepolicy=True, virtualinteraction=True):
        ...
        for self.total_step in range(1, num_frames + 1):
            ...
            next_state1, reward1, done1, violation1, violation_M1, violation_N1, voltage_M1, voltage_N1, grid_loss1, new_state1 = self.step_model(action_0)
            self.transition = [self.state, action, reward, next_state, done]
            self.memory.store(*self.transition)
            reward_4 += reward
            if self.env.step_n % 4 == 0:
                self.memory.store_1(reward_4 / 4)
                reward_4 = 0
        ...
        data_train.to_csv('train' + str(self.env.name) + str(self.seed) + 'two' + str(self.partical) + '4s.csv')
        ...
        data_train_test.to_csv('traintest' + str(self.env.name) + str(self.seed) + 'two'+ str(self.partical) +'4s.csv')
        ...
        data_train_loss.to_csv('trainloss' + str(self.env.name) + str(self.seed)+ 'two'+ str(self.partical) +'4s.csv')
    ...
    def test(self, test_frams, real_model = False, safepolicy = False):
        ...
        data_test.to_csv('test'+str(self.env.name)+str(self.seed) +'two'+'.csv')
        ...
        data_test_step.to_csv('test_step' + str(self.env.name) + str(self.seed) +'two'+ '.csv')

        data_test_step_action = DataFrame(actions)
        data_test_step_action.to_csv('test_step_action' + str(self.env.name) + str(self.seed)  +'two'+ '.csv')
