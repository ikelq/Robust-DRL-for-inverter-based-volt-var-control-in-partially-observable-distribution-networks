...
class grid_case:
    ...
    def __init__(self,
        ...
        self.topology_change = np.array([1,0])
        ...
        self.observation_space = copy.deepcopy(np.hstack(
            (np.array(self.model.res_bus.vm_pu[[0]+self.id_iber + self.id_svc]), np.array(self.model.res_ext_grid.iloc[0,0]), np.array(self.model.res_sgen.q_mvar),
    ...
    def step_model(self,
        ...
        # switch line 33 and line 8
        # line 33 from bus 8 to bus 14
        # line 8 from bus 8 to bus 9
            self.topology_change = np.array([0,1])
            self.model.line.loc[33, 'in_service'] = True
            self.model.line.loc[8, 'in_service'] = False
        ...
        self.model.sgen.loc[:len(self.id_iber)-1, 'p_mw'] = self.gene_pu[self.step_n-1]
        #
        ...
        next_state = np.hstack((np.array(self.model.res_bus.vm_pu[[0]+self.id_iber + self.id_svc]),np.array(self.model.res_ext_grid.iloc[0,0]),action,
                                self.topology_change))
        ...
        self.model.sgen.q_mvar = action
        self.model.sgen.loc[:len(self.id_iber)-1, 'p_mw'] = self.gene_pu[self.step_n]
        pp.runpp(self.model, algorithm='bfsw')

        new_state = np.hstack((np.array(self.model.res_bus.vm_pu[[0]+self.id_iber + self.id_svc]),np.array(self.model.res_ext_grid.iloc[0,0]),action,
                               self.topology_change))
