...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim: int, action_dim: int, size: int, batch_size: int = 32):
        ...
        self.rews_buf = np.zeros([size, decay_average, size, 2], dtype=np.float32)
        self.done_buf = np.zeros([size, decay_average], dtype=np.float32)
...
class SACAgent:
    ...
    def __init__(
        ...
        self.qf_1_optimizer = SAM(self.qf_1.parameters(), torch.optim.Adam, lr=3e-4)
        self.qf_2_optimizer = SAM(self.qf_2.parameters(), torch.optim.Adam, lr=3e-4)
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        q1_target = reward[:, :, 0].mean(1)
        q2_target = 50*reward[:, , 1].mean(1)
    ...
    def train(self, num_frames: int, plotting_interval: int = 400):
        ...
        for self.total_step in range(1, num_frames + 1):
            ...
            if self.env.step_n % decay_average == 0:
                ...
                stateB, actionB, rewardB, next_stateB, doneB = \
                    np.vstack((stateB, self.state)), np.vstack((actionB,action)), np.vstack((rewardB, reward)), np.vstack((next_stateB,next_state)), np.vstack((next_stateB,done))
        ...
        data_train.to_csv('trainsac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+'sam.csv')
        ...
        data_train_test.to_csv('traintestsac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+'sam.csv')
        ...
        data_train_loss.to_csv('trainlosssac' + str(self.env.name) + str(self.seed) + str(self.partical) + 'tootsa'+str(qr)+str(decay)+str(decay_average)+'sam.csv')
        #
